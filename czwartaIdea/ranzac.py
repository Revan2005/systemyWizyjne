...
def pupil_position_ranzac(thresh, (x0, y0), (pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    ...
    pupilX, pupilY, rCircle = getPupilPosition(oko_thresh)
    pupilX += x0 - crop_width / 2
    pupilY += y0 - crop_height / 2
    
    cv2.namedWindow("Ranzack circle")
    cv2.moveWindow("Ranzack circle", 100, 50)
    podglad = thresh.copy()
    cv2.circle(podglad, (pupilX, pupilY), rCircle, (255,0,0), 1)
    cv2.imshow("Ranzack circle", podglad)
...
def poprawka_na_krawedz_ekranu((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    max_pupil_x = cap_width - crop_width / 2 - 5  
    min_pupil_x = crop_width / 2 + 5
    max_pupil_y = cap_height - crop_height / 2 - 5
    min_pupil_y = crop_height / 2 + 5
    if not czyMoznaWPrawo((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
        pupilX = max_pupil_x
    elif not czyMoznaWLewo((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
        pupilX = min_pupil_x
    if not czyMoznaWGore((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
        pupilY = min_pupil_y
    elif not czyMoznaWDol((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
        pupilY = max_pupil_y
    return (pupilX, pupilY)

def czyMoznaWPrawo((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    if pupilX < cap_width - crop_width / 2 - 5:
        return True
    return False

def czyMoznaWLewo((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    if pupilX > cap_width / 2 + 5:
        return True
    return False

def czyMoznaWGore((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    if pupilY > crop_height / 2 + 5:
        return True
    return False

def czyMoznaWDol((pupilX, pupilY), (crop_width, crop_height), (cap_width, cap_height)):
    if pupilY < cap_height - crop_height / 2 - 5:
        return True
    return False
